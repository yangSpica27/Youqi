...
public class RandomTextView extends AppCompatTextView {
  ...
  private void drawText(Canvas mCanvas, String text, float x, float y, Paint p) {
    if (y >= -measuredHeight && y <= 2 * measuredHeight) { mCanvas.drawText(text + "", x, y, p); } else return;
  }

  private int getDrawX(int j) {
    if (j <= pointIndex || pointIndex == -1) { return (int) (fontWidth * j); } else return (int) (fontWidth * (j - 1) + pointWidth);
  }
